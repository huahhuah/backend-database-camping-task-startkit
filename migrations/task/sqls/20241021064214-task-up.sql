...
FROM 
    "COACH" c
CROSS JOIN 
    (SELECT id FROM "SKILL" s WHERE name = '重訓') s 
WHERE 
    c.user_id IN (  
...
VALUES 
  (
    ...
    (SELECT idFROM "SKILL"WHERE "name" = '有氧運動')
  ),
  (
    (SELECT id FROM "COACH" WHERE "user_id" = (SELECT id FROM "USER" WHERE "email" = 'starplatinum@hexschooltest.io')),
...
from "COURSE_BOOKING" 
where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
  and status != '課程已取消' group by "COURSE_BOOKING".user_id;
...
SELECT 
    ...
    (SUM(cp.purchASed_credits) - 
     (SELECT COUNT(*) FROM "COURSE_BOOKING" WHERE user_id = cp.user_id AND status IN ('上課中', '已完成') )) AS remaining_credit
